...
def get_filter_by_selected_menu(filter_num):
    filters = {
        0: None,
        1: {
            "option": "S",
...
def get_filtering_state(selected_menu, user=None, course_id=None, subject_id=None):
    filtering_state = get_filter_by_selected_menu(selected_menu)
    if selected_menu == 1:
        try:
            if course_id:
                try:
                    filtering_state["topic"] = Course.objects.get(
                        id=course_id
                    ).name

                    if subject_id:
                        filtering_state["subtopic"] = Forum.objects.get(
                            id=subject_id
                        ).name
                except Course.DoesNotExist:
                    filtering_state["topic"] = Profile.objects.get(
                        user=user
                    ).course.name
            else:
                filtering_state["topic"] = Profile.objects.get(
...
def get_courses():
    return [course.as_dict() for course in Course.objects.all()]
    ...
    if filtering_state:
        if filtering_state["option"] and filtering_state["option"] != "":
            if filtering_state["option"] == "O":
                ...
                if filtering_state["subtopic"]:
                    questions = questions.filter(
                        forum__name=filtering_state["subtopic"]
                    )
    else:
...
def add_question(author, topic_id=None, forum_id=None, description="", title=""):
    ...
    if topic_id:
        ...
        question = Question(forum=topic.forum, topic=topic,
                            description=description, author=author, title=title)
    elif forum_id:
        try:
        ...
        question = Question(
            forum=forum, description=description, author=author, title=title)
